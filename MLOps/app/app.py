...
from utils.actions import notificar_crm, gerar_parecer_agente, notificar_analista

sys.stderr.write("[APP] imports concluidos\n")
sys.stderr.flush()


# __file__ = /app/MLOps/app/app.py → .parent.parent.parent = /app (raiz do projeto)
_METADATA_PATH = Path(__file__).resolve().parent.parent.parent / "Model" / "artifacts" / "metadata_modelo.json"
...
if analisar:
    ...
    # ── Classificação + ação automatizada por faixa de risco ─────────────────
    if probabilidade < 30:
        ...
        notificar_crm(inputs, resultado["probability"])
        acao_auto = {
            "icone": "✅",
            "titulo": "Ação Automatizada — BAIXO RISCO",
            "corpo": "Notificação de aprovação disparada ao CRM/ERP via webhook.",
        }
        ...
        with st.spinner("🤖 Consultando agente de IA para análise complementar..."):
            parecer = gerar_parecer_agente(inputs, resultado["probability"])
        acao_auto = {
            "icone": "🤖",
            "titulo": "Parecer do Agente de IA — MÉDIO RISCO",
            "corpo": parecer,
        }
    else:
        ...
        notificar_analista(inputs, resultado["probability"])
        acao_auto = {
            "icone": "🔔",
            "titulo": "Ação Automatizada — ALTO RISCO",
            "corpo": "Analista notificado automaticamente (Slack ou e-mail). Caso adicionado à fila de revisão manual.",
        }
...
""", unsafe_allow_html=True)
    ...
    # ── Seção de Ação Automatizada ────────────────────────────────────────────
    _aa = _res.get("acao_auto")
    if _aa:
        with st.expander(f"{_aa['icone']} {_aa['titulo']}", expanded=True):
            st.write(_aa["corpo"])
